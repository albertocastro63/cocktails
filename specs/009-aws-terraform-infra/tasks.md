...
- [X] T002 [P] Create `infra/versions.tf` with `terraform { required_version = ">= 1.10" }` block, `required_providers` pinning `hashicorp/aws ~> 5.0`, and a `provider "aws"` block with `region = var.aws_region` and `default_tags { tags = { project = var.project_name, environment = var.environment, managed-by = "terraform" } }`
