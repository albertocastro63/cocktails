...
**Purpose**: The `IsBaseSpirit` field must exist on `model.Ingredient` and on the DynamoDB `ingItem` struct before any frontend or handler work can reference or serialize it. Both tasks touch different files and can run in parallel.

**⚠️ CRITICAL**: No user story work can begin until this phase is complete.

- [ ] T001 [P] Add `IsBaseSpirit bool \`json:"is_base_spirit,omitempty"\`` to `Ingredient` struct in `backend/internal/model/model.go`
- [ ] T002 [P] Add `BaseSpirit bool \`dynamodbav:"is_base_spirit,omitempty"\`` to `ingItem` struct and update `toItem`/`fromItem` mapping functions in `backend/internal/store/dynamo/recipes.go`

**Checkpoint**: `go build ./...` passes. The field is present in both the JSON API shape and the DynamoDB attribute envelope.
...
> **Write tests FIRST. Confirm they FAIL before implementing T004–T007.**

- [ ] T003 [US1] Write failing tests for base spirit toggle behaviour in `frontend/src/pages/RecipeForm.test.js`:
...
- [ ] T004 [US1] Update the ingredient row builder in `frontend/src/pages/RecipeForm.js` to append a base spirit checkbox `<input type="checkbox" name="ing_base_spirit">` with an amber label to each ingredient row (after the Unit input, before the × button)
- [ ] T005 [US1] Add mutual-exclusion click handler in `frontend/src/pages/RecipeForm.js`: when a base spirit checkbox is checked, find all sibling ingredient rows via `ingredientsSection._rows.children` and uncheck their base spirit inputs; when unchecked, leave all others as-is
- [ ] T006 [US1] Update the edit prefill block in `frontend/src/pages/RecipeForm.js` so that when loading an existing recipe, the ingredient row with `ing.is_base_spirit === true` has its checkbox set to `checked`
- [ ] T007 [US1] Update the submit handler in `frontend/src/pages/RecipeForm.js` to read `row.querySelector('[name="ing_base_spirit"]').checked` for each ingredient row and include `is_base_spirit: true` in the payload only when `checked`
...
> **Write tests FIRST. Confirm they FAIL before implementing T009.**

- [ ] T008 [US2] Write failing tests for base spirit highlight in popover in `frontend/src/components/RecipeCard.test.js`:
  - When one ingredient has `is_base_spirit: true`, the rendered popover list item contains a `(base spirit)` label or `font-bold` class
...
- [ ] T009 [US2] Update `buildIngredientPopover()` in `frontend/src/components/RecipeCard.js`: when rendering each ingredient `li`, if `ingredient.is_base_spirit` is truthy apply `font-semibold text-stone-900` to the name span and append `<span class="text-amber-600 text-xs ml-1">(base spirit)</span>` inline
...
> **Write tests FIRST. Confirm they FAIL before implementing T011.**

- [ ] T010 [US3] Write failing tests for base spirit highlight in `frontend/src/components/IngredientList.test.js`:
  ...
  - Visual treatment matches the popover pattern (same class/label strings)

### Implementation for User Story 3

- [ ] T011 [US3] Update `IngredientList.js` in `frontend/src/components/IngredientList.js`: when rendering each ingredient `li`, if `ingredient.is_base_spirit` is truthy apply `font-semibold text-stone-900` to the name span and append `<span class="text-amber-600 text-xs ml-1">(base spirit)</span>` — identical token usage to T009
...
- [ ] T012 Run all backend tests and confirm they pass: `cd backend && go test ./...`
- [ ] T013 [P] Run all frontend tests and confirm they pass: `cd frontend && npm test`
- [ ] T014 [P] Manually validate quickstart.md scenarios 1–7 against the running dev server
...
T003 → T004 → T005 → T006 → T007  # sequential (same file: RecipeForm.js)
...
# Phase 6 — T013 and T014 parallel with each other (after T012):
T012 → T013 [P]
      T014 [P]
```
...
- The mutual-exclusion logic in T005 operates on the live DOM (iterating `ingredientsSection._rows.children`) so it works correctly even when ingredient rows are added or removed during editing.
- The visual treatment (T009, T011) uses identical class strings and label text to satisfy FR-008 (consistency between popover and detail page); keep them in sync if either is changed later.
