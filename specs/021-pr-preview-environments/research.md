...
**Decision**: Introduce a `VITE_API_PATH_PREFIX` build-time env var (default: `/api`) in `frontend/src/api/client.js`. All API calls become `${VITE_API_PATH_PREFIX}/v1/...`. Production builds use the default. Preview builds use `/api/pr-42`.
...
**Rationale**: The API Gateway route for PR previews is `ANY /api/pr-42/{proxy+}`. API Gateway forwards the full original path (e.g., `/api/pr-42/v1/recipes`) to the Lambda. The existing Go mux patterns are registered as `/api/v1/...`. Stripping `/pr-42` before routing transforms the path back to what the mux expects — with no changes to handler registrations.
...
**Decision**: CI scripts use `aws dynamodb create-table` to provision three per-PR tables (`cocktails-pr-42-recipes`, `cocktails-pr-42-users`, `cocktails-pr-42-favorites`). Seeding copies items from production tables using `aws dynamodb scan` + `aws dynamodb batch-write-item`. Seeding is skipped on subsequent pushes to the same PR (idempotent check: if tables exist, skip creation and seeding).

**Rationale**: DynamoDB's on-demand billing means tables cost nothing until accessed. Scan + batch-write is simple and requires no external tooling. The idempotent check (table exists → skip) enables subsequent PR pushes to update code only, consistent with the spec's seed-once requirement.

**Alternatives considered**:
- DynamoDB export/import (S3 export format) — rejected; adds S3 intermediate step and is slower for small datasets.
- Seeding from the recipe export API endpoint — considered but rejected because the export only covers recipes, not users or favorites.
...
**Rationale**: HTTP API v2 supports multiple explicit routes alongside the `$default` catch-all. An explicit route `ANY /api/pr-42/{proxy+}` is more specific than `$default` and takes priority. CI manages these routes without Terraform to avoid state file pollution.
